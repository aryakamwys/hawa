...
export default function MapPollutan() {
  ...
  const [mapHeight, setMapHeight] = useState(() => (window.innerWidth < 768 ? '420px' : '640px'));
  const token = authService.getToken();
  const apiUrl = useMemo(() => import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000', []);

  useEffect(() => {
    const onResize = () => {
      setMapHeight(window.innerWidth < 768 ? '420px' : '640px');
    };
        ...
        <main className="flex-1 overflow-y-auto bg-gray-50">
          <div className="px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className={`lg:col-span-2 ${isSidebarOpen ? 'hidden lg:block' : 'block'}`}>
              ...
              <div className="lg:col-span-1 space-y-6">
                <HeatmapInfo apiUrl={apiUrl} token={token} language="id" />
                <HeatmapTips
                  apiUrl={apiUrl}
                  token={token}
